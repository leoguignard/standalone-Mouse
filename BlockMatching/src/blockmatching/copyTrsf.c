/*************************************************************************
 * copyTrsf.c - template for executable creation
 *
 * $Id$
 *
...
int main( int argc, char *argv[] )
{
   ...
   */
  if ( argc <= 1 )
      API_ErrorParse_copyTrsf( _BaseName( argv[0] ), (char*)NULL, 0 );
  API_ParseParam_copyTrsf( 1, argc, argv, &par );
  ...
  if ( lineoptions == (char*)NULL )
      API_ErrorParse_copyTrsf( _BaseName( argv[0] ), "unable to translate command line options ...\n", 0 );

  if ( API_INTERMEDIARY_copyTrsf( par.thetrsf_name,
                                  par.restrsf_name,
                                  par.template_image_name,
                                  par.floating_image_name,
                                  lineoptions, (char*)NULL ) != 1 ) {
      vtfree( lineoptions );
      API_ErrorParse_copyTrsf( _BaseName( argv[0] ), "some error occurs during processing ...\n", -1 );
  }
  vtfree( lineoptions );
