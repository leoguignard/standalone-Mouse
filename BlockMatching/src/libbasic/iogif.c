...
void *_readGifImage( char *name, int *dimx, int *dimy, int *dimz )
{
  ...
  *dimx = *dimy = *dimz = 0;
  ...
  vtfree(RawGIF);	 RawGIF = NULL;   /* We're done with the raw data now */
...
static void * GifError(char *st) {
  ...
  if (RawGIF != (byte*)NULL) vtfree(RawGIF);
  if (Raster != (byte*)NULL) vtfree(Raster);
  
  if ( fd != (FILE*)NULL )   fclose( fd );
  
  if ( theBuf != (void*)NULL ) vtfree( theBuf );
