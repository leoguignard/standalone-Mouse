                                         ...
                                         int dimx, int dimy, int b )
{
  ...
  if ( s <= 0 ) return( 0 );
  tmp = (unsigned char *)vtmalloc( s * 3 * b * sizeof(unsigned char), "tmp", proc );
                   ...
                   int *dimx, int *dimy, int *dimz, int *bytes )
{
  ...
  *bytes = 0;
  ...
  buf = (void*)vtmalloc( sizeOfBuffer * sizeof( unsigned char ), "buf", proc );
  if ( buf == (void*)NULL ) {
    if ( _verbose_ )
                     ...
                     int x, int y, int z, int b, void *buf )
{
  ...
  case 1 :
    if ( _WriteGreyImagesAsColorOnes_ != 0 ) {
      ...
      bufToWrite = (unsigned char *)vtmalloc( x * y * 3 * b * sizeof(unsigned char),
                                              "bufToWrite", proc );
  ...
  if ( f == (FILE*)NULL ) {
    if ( _verbose_ )
      fprintf( stderr, "%s: error in opening %s\n", proc, name );
    if ( _WriteGreyImagesAsColorOnes_ != 0 ) vtfree( bufToWrite );
    return;
  }
